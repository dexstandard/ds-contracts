 ...
 *         Strict version to work with DCA bot order manager.
 */
contract VaultStrict is ERC20Upgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    event OrderManagerChanged(address indexed oldManager, address indexed newManager);
    ...
    function depositFrom(address _from, uint256 _amount, address _sharesHolder)
        external
        nonReentrant
        onlyOrderManager
     ...
     */
    function withdrawFrom(uint256 _shares, address _receiver, address _sharesHolder)
        public
        nonReentrant
        onlyOrderManager
        returns (uint256 _amountOut)
    {
        ...
        uint256 amountOut = (totalBal * _shares) / totalSupply();

        _burn(_sharesHolder, _shares);
        ...
        if (vaultBal < amountOut) {
            uint256 toWithdraw = amountOut - vaultBal;
            ...
            if (diff < toWithdraw) {
                amountOut = vaultBal + diff;
            }
        }

        // send baseToken to the receiver
        IERC20(baseToken).safeTransfer(_receiver, amountOut);
    ...
    /**
     * @dev Allows the owner to set a new order manager.
     */
    function setOrderManager(address _orderManager) external onlyOwner {
        require(_orderManager != address(0), "Invalid manager address");
        emit OrderManagerChanged(orderManager, _orderManager);
        orderManager = _orderManager;
    }
