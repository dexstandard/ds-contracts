...
import {IOnchainOracle} from "../interfaces/swapper/IOnchainOracle.sol";
import {BytesLib} from "./BytesLib.sol";

contract OnchainSwapper is OwnableUpgradeable {
    ...
    function _executeSwap(address _fromToken, address _toToken, uint256 _amountIn, uint256 _minAmountOut) private {
        ...
        bytes memory minAmountData =
            swapData.minAmountSign >= 0 ? abi.encode(_minAmountOut) : abi.encode(-int256(_minAmountOut));

        data = _insertData(data, swapData.minIndex, minAmountData);

        IERC20(_fromToken).forceApprove(router, type(uint256).max);
    ...
    function _getFreshPrice(address _fromToken, address _toToken)
        private
